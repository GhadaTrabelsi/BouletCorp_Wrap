...
# Selenium - Méthode wrap dans Selenium
La méthode wrap est une fonctionnalité fournie par Selenium qui permet de créer un objet WebElement à partir d'un élément de la page web. 
Cette méthode est utilisée en conjonction avec le modèle de conception Page Object et facilite l'interaction avec les éléments de la page.

Lorsque vous utilisez la méthode wrap, vous fournissez un élément de la page (par exemple, une balise input, 
un bouton, une zone de texte, etc.) et la méthode crée un objet WebElement à partir de cet élément. L'objet WebElement ainsi créé vous permet 
d'accéder aux propriétés et méthodes spécifiques à cet élément, ce qui facilite la manipulation et les interactions avec la page.
